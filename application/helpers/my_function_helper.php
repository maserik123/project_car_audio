...
function _sendEmail($tujuan, $password)
{
    ...
    $CI->email->from('', 'LSP Information System');
    $CI->email->to($tujuan);
    $CI->email->subject('Change Password Sistem Informasi LSP Politeknik Caltex Riau');
    $CI->email->message('Hello ' . $CI->session->userdata('first_name') . ' ' . $CI->session->userdata('last_name') . '<br> Password anda adalah ' . $password . '<br> Segera ubah password anda melalui sistem LSP.');
    if ($CI->email->send()) {
        return true;
    } else {
